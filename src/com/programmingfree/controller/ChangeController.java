...
public class ChangeController extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		if(request.getParameter("action")!=null){
			List<ChangeTracker> lstChange=new ArrayList<ChangeTracker>();
			...
			if(action.equals("list")){
				...
				//Fetch Data from User Table
					int startPageIndex=
					       ...
					       lstChange=dao.getAllChangeTrackers(startPageIndex,numRecordsPerPage);
					       //Get Total Record Count for Pagination
					       int userCount=dao.getChangeTrackerCount();			
				//Convert Java Object to Json				
				JsonElement element = gson.toJsonTree(lstChange, new TypeToken<List<ChangeTracker>>() {}.getType());
				...
				try{											
					if(action.equals("create")){//Create new record
						...
						dao.updateChangeTracker(change);
				...
				try{
					if(request.getParameter("idchangetracker")!=null){
						...
						dao.deleteChangeTracker(Integer.parseInt(idchangetracker));
						String listData="{\"Result\":\"OK\"}";								
						response.getWriter().print(listData);
					}
				}catch(Exception ex){
